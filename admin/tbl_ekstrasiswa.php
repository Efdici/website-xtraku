...
<body>
    ...
    <table width="90%" style="margin-top: 1%; margin-left: 5%;"  border="1" cellpadding="0" cellspacing="0">
        <tr bgcolor="#FFA600" height="40">
             <th width="12%"><center>Id EkstraSiswa</center></th>&nbsp;
...
while ( $hasil             = mysqli_fetch_array ($Tampil)) {
        $id_ekstrasiswa         = stripslashes  ($hasil['id_ekstrasiswa']);
        $id_ekstra         = stripslashes  ($hasil['id_ekstra']);
        $id_siswa          = stripslashes  ($hasil['id_siswa']);
        {
?>
        <tr align="center" height="40">
            <td><?=$id_ekstrasiswa?></td>
            <td><?=$id_ekstra?></td>
            <td><?=$id_siswa?></td>
            <td>
                <a onclick="return confirm('tekan OKE untuk menghapus')" href="hapus_ekstrasiswa.php?id_ekstrasiswa=<?php echo $hasil['id_ekstrasiswa'];?>" style="padding:1% 1%;background-color: #B22222;color: #fff;border-radius: 2px;text-decoration: none;">Delete</a>
                <a href="edit_ekstrasiswa.php?id_ekstrasiswa=<?php echo $hasil['id_ekstrasiswa'];?>" style="padding:1% 1%;background-color: #1E90FF;color: #fff;border-radius: 2px;text-decoration: none;">Update</a>
            </td>
<?php }} ?>
    </table>
</body>
